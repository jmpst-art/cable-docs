...
Cable includes a prebuilt action that is able to edit Eloquent records. When the trigger button is clicked, a modal will open with a form inside. The user fills the form, and that data is validated and saved into the database. You may use it like so:

```php
use Cable\Actions\EditAction;
...
If you want to edit table rows, you can use the `Cable\Tables\Actions\EditAction` instead:

```php
use Cable\Forms\Components\TextInput;
use Cable\Tables\Actions\EditAction;
