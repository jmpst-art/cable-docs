...
Cable includes a prebuilt action that is able to create Eloquent records. When the trigger button is clicked, a modal will open with a form inside. The user fills the form, and that data is validated and saved into the database. You may use it like so:

```php
use Cable\Actions\CreateAction;
...
If you want to add this action to the header of a table instead, you can use `Cable\Tables\Actions\CreateAction`:

```php
use Cable\Forms\Components\TextInput;
use Cable\Tables\Actions\CreateAction;
...
use Cable\Forms\Components\Wizard\Step;
