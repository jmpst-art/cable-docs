...
```bash
composer require cable/actions:"^3.0"
```
...
```bash
php artisan cable:install --scaffold --actions
npm install
npm run dev
```
...
```bash
php artisan cable:install --actions
```
...
In `resources/css/app.css`, import Tailwind CSS, and the Cable forms CSS, which is used for action modal forms:

```css
@import '../../vendor/cable/forms/dist/index.css';
...
```bash
php artisan vendor:publish --tag=cable-actions-config
```
