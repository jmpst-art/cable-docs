...
```bash
php artisan make:cable-resource Customer
```
...
```bash
php artisan make:cable-resource Customer --simple
```
...
If you'd like to save time, Cable can automatically generate the [form](#forms) and [table](#table) for you, based on your model's database columns.
...
```bash
php artisan make:cable-resource Customer --generate
```

> Note: If your table contains ENUM columns, `doctrine/dbal` is unable to scan your table and will crash. Hence Cable is unable to generate the schema for your resource if it contains an ENUM column. Read more about this issue [here](https://github.com/doctrine/dbal/issues/3819#issuecomment-573419808).
...
```bash
php artisan make:cable-resource Customer --soft-deletes
```
...
```bash
php artisan make:cable-resource Customer --view
```
...
Check out the Forms docs for a [guide](../../forms/getting-started) on how to build forms with Cable.
...
For authorization, Cable will observe any [model policies](https://laravel.com/docs/authorization#creating-policies) that are registered in your app. The following methods are used:
...
- `delete()` is used to prevent a single record from being deleted. `deleteAny()` is used to prevent records from being bulk deleted. Cable uses the `deleteAny()` method because iterating through multiple records and checking the `delete()` policy is not very performant.
- `forceDelete()` is used to prevent a single soft-deleted record from being force-deleted. `forceDeleteAny()` is used to prevent records from being bulk force-deleted. Cable uses the `forceDeleteAny()` method because iterating through multiple records and checking the `forceDelete()` policy is not very performant.
- `restore()` is used to prevent a single soft-deleted record from being restored. `restoreAny()` is used to prevent records from being bulk restored. Cable uses the `restoreAny()` method because iterating through multiple records and checking the `restore()` policy is not very performant.
...
{
    return __('cable/resources/customer.label');
}
```
...
{
    return __('cable/resources/customer.plural_label');
}
```

## Navigation

Cable will automatically generate a navigation menu item for your resource using the [plural label](#plural-label).
...
{
    return __('cable/resources/customer.navigation_label');
}
```
...
{
    return __('cable/navigation.groups.shop');
}
```

## Customizing the Eloquent query

Within Cable, every query to your resource model will start with the `getEloquentQuery()` method.
...
By default, Cable will observe all global scopes that are registered to your model. However, this may not be ideal if you wish to access, for example, soft deleted records.

To overcome this, you may override the `getEloquentQuery()` method that Cable uses:
...
By default, Cable will generate a URL based on the name of the resource. You can customize this by setting the `$slug` property on the resource:
