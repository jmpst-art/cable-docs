...
Cable allows you to build dynamic dashboards, comprised of "widget" cards, very easily.

The following document will explain how to use these widgets to assemble a dashboard using the app framework.

## Available widgets

Cable ships with these widgets:
...
You may also [create your own custom widgets](#custom-widgets) which can then have a consistent design with Cable's prebuilt widgets.
...
Now, in your new `app/Cable/Pages/Dashboard.php` file, you may override the `getColumns()` method to return a number of grid columns to use:
...
```bash
php artisan make:cable-widget LatestOrders --table
```
...
```bash
php artisan make:cable-widget BlogPostsOverview
```

This command will create two files - a widget class in the `/Widgets` directory of the Cable directory, and a view in the `/widgets` directory of the Cable views directory.
...
If you want to customize the dashboard class, for example to [change the number of widget columns](#customizing-widget-width), create a new file at `app/Cable/Pages/Dashboard.php`:

```php
<?php

namespace App\Cable\Pages;

use Cable\Pages\Dashboard as BasePage;
