...
```php
use Cable\Forms\Components\Actions\Action;
use Cable\Forms\Components\TextInput;
...
In Cable, you may interact with the `Mask` object in the `mask()` method to configure your mask:
...
Under the hood, masking is powered by [`imaskjs`](https://imask.js.org). The vast majority of its masking features are also available in Cable. Reading their [guide](https://imask.js.org/guide.html) first, and then approaching the same task using Cable is probably the easiest option.
