...
```bash
composer require cable/tables:"^3.0"
```
...
```bash
php artisan cable:install --scaffold --tables
npm install
npm run dev
```
...
```bash
php artisan cable:install --tables
```
...
```bash
php artisan vendor:publish --tag=cable-tables-config
```
